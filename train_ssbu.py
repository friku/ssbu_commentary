...
def main():
    ...
    ###
    # tokenizer=model.gpt.tokenizer
    # print(tokenizer.tokenize('ひまわり が オレンジ 色 の 花瓶 に 生け て 置い て ある'))
    # print(tokenizer('ひまわり が オレンジ 色 の 花瓶 に 生け て 置い て ある').input_ids)
    # print(tokenizer.tokenize('ひまわりがオレンジ色の花瓶に生けて置いてある'))
    # print(tokenizer('ひまわりがオレンジ色の花瓶に生けて置いてある').input_ids)
    # print(tokenizer.encode('ひまわり が オレンジ 色 の 花瓶 に 生け て 置い て ある'.split(' ')))
    # print(tokenizer.encode('ひまわりがオレンジ色の花瓶に生けて置いてある'.split(' ')))
    # print(tokenizer.encode('ひまわりがオレンジ色の花瓶に生けて置いてある'))
    # print(tokenizer.encode('フォックス側はひとたまりもないですが'))
    # print(tokenizer.tokenize('フォックス側はひとたまりもないですが'))
    ...
    train_dataset = SSBUCaptionDataset(tokenizer=model.gpt.tokenizer, clip_preprocess=model.clip.preprocess, split="train", prefix_length=args.prefix_length)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

    val_dataset, val_dataloader = None, None
    val_dataset = SSBUCaptionDataset(tokenizer=model.gpt.tokenizer, clip_preprocess=model.clip.preprocess, split="val", prefix_length=args.prefix_length)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=True)

    test_dataset, test_dataloader = None, None
    test_dataset = SSBUCaptionDataset(tokenizer=model.gpt.tokenizer, clip_preprocess=model.clip.preprocess, split="test", prefix_length=args.prefix_length)
